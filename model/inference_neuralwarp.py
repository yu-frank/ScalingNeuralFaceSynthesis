...
class NeuralWarp_Mask(nn.Module):
    ...
    def forward(self, nn_features, uv_map_2, uv_map_1, extrinsics_2, skip_connection, neural_texture, pose_2, pose_1):
        cache_sample = nn_features

        if self.sh_skips and self.cache_features:
            skip_basis_1 = self._spherical_harmonics_basis(pose_1)
            skip_basis_1 = skip_basis_1.view(skip_basis_1.shape[0], skip_basis_1.shape[1], 1, 1)
            skip_basis_2 = self._spherical_harmonics_basis(pose_2)
